...
XilinxOMXPlugin *createOMXPlugin() {
    return new XilinxOMXPlugin;
}

XilinxOMXPlugin::XilinxOMXPlugin()
    : mLibHandle(dlopen("libOMX.allegro.core.so", RTLD_NOW)),
      mInit(NULL),
      mDeinit(NULL),
...
OMX_ERRORTYPE XilinxOMXPlugin::makeComponentInstance(
        const char *name,
...
OMX_ERRORTYPE XilinxOMXPlugin::destroyComponentInstance(
        OMX_COMPONENTTYPE *component) {
    if (mLibHandle == NULL) {
        return OMX_ErrorUndefined;
    }
...
OMX_ERRORTYPE XilinxOMXPlugin::enumerateComponents(
        OMX_STRING name,
        size_t size,
        OMX_U32 index) {
    if (mLibHandle == NULL) {
...
OMX_ERRORTYPE XilinxOMXPlugin::getRolesOfComponent(
        const char *name,
